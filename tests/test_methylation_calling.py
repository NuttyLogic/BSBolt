...
with open(f'{bsb_directory}tests/TestSimulations/BSB_pe.chr11_values.pkl', 'rb') as sim_sites:
    chr11_methylation_sites = pickle.load(sim_sites)
...
def get_vector_point_values(vector, value_dict):
    for meth_call, pos in zip(vector[3], vector[4]):
        if pos in value_dict:
            if meth_call:
                value_dict[pos][0] += 1
            else:
                value_dict[pos][1] += 1
...
for vector in all_vectors:
    get_vector_point_values(vector, vector_point_values)
...
for site, values in vector_point_values.items():
    ...
    try:
        reference_values = chr11_methylation_sites[f'chr11:{site}']
    except KeyError:
        # simulated SNP if not in reference values
        continue
    ...
    def test_beta_proportion(self):
        ...
        for test_site in site_comparisons.values():
            if test_site['beta_z_value'] >= z_threshold:
                z_site_count += 1
        self.assertLessEqual(z_site_count, 100)
