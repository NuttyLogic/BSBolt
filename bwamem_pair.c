...
int mem_sam_pe(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const uint8_t *opac, const mem_pestat_t pes[4], uint64_t id, bseq1_t s[2], mem_alnreg_v a[2])
{
	extern int mem_mark_primary_se(const mem_opt_t *opt, int n, mem_alnreg_t *a, int64_t id);
	extern int mem_approx_mapq_se(const mem_opt_t *opt, const mem_alnreg_t *a);
	extern void mem_reg2sam(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const uint8_t *opac, bseq1_t *s, mem_alnreg_v *a, int extra_flag, const mem_aln_t *m);
	extern char **mem_gen_alt(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const uint8_t *opac, const mem_alnreg_v *a, int l_query, const char *query, const char *oquery);
	...
	if (n_pri[0] && n_pri[1] && (o = mem_pair(opt, bns, pac, pes, s, a, id, &subo, &n_sub, z, n_pri)) > 0) {
		...
		if (!(opt->flag & MEM_F_ALL)) {
			for (i = 0; i < 2; ++i)
				XA[i] = mem_gen_alt(opt, bns, pac, opac, &a[i], s[i].l_seq, s[i].seq, s[i].oseq);
		} else XA[0] = XA[1] = 0;
		// write SAM
		for (i = 0; i < 2; ++i) {
			h[i] = mem_reg2aln(opt, bns, pac, opac, s[i].l_seq, s[i].seq, &a[i].a[z[i]], s[i].oseq);
			h[i].mapq = q_se[i];
			...
			if (n_pri[i] < a[i].n) { // the read has ALT hits
				...
				g[i] = mem_reg2aln(opt, bns, pac, opac, s[i].l_seq, s[i].seq, p, s[i].oseq);
...
no_pairing:
	for (i = 0; i < 2; ++i) {
		...
		if (which >= 0) h[i] = mem_reg2aln(opt, bns, pac, opac, s[i].l_seq, s[i].seq, &a[i].a[which], s[i].oseq);
		else h[i] = mem_reg2aln(opt, bns, pac, opac, s[i].l_seq, s[i].seq, 0, s[i].oseq);
	}
	...
	mem_reg2sam(opt, bns, pac, opac, &s[0], &a[0], 0x41|extra_flag, &h[1]);
	mem_reg2sam(opt, bns, pac, opac, &s[1], &a[1], 0x81|extra_flag, &h[0]);
	if (strcmp(s[0].name, s[1].name) != 0) err_fatal(__func__, "paired reads have different names: \"%s\", \"%s\"\n", s[0].name, s[1].name);
