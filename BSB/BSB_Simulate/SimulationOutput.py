...
class SimulationOutput:
    ...
    def __init__(self, simulation_output=None, reference_contigs=None):
    ...
    def load_contig(self, contig_id):
        with open(f'{self.simulation_output}.{contig_id}.pkl', 'rb') as contig_out:
            contig_methylation = pickle.load(contig_out)
            return contig_methylation

    def load_contig_key(self):
        with open(f'{self.simulation_output}.genome_index.pkl', 'rb') as ref_out:
